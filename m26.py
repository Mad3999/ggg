...
def load_historical_data(symbol, period="3mo", force_refresh=False):
    ...
    Args:
        symbol (str): Symbol to load historical data for
        period (str): Period to fetch (e.g., "3mo" for 3 months)
        force_refresh (bool): Force refresh even if data exists
    ...
    # Skip if already has recent data and not forcing refresh
    if not force_refresh and symbol in stocks_data:
        history_df = stocks_data[symbol]["price_history"]
        if not history_df.empty and len(history_df) > 50:
            try:
                last_timestamp = history_df['timestamp'].max()
                if last_timestamp:
                    # If we have data from the last day, skip refresh
                    today = pd.Timestamp.now().floor('D')
                    if isinstance(last_timestamp, str):
                        try:
                            last_timestamp = pd.Timestamp(last_timestamp)
                        except:
                            # If conversion fails, continue with refresh
                            pass
                    if isinstance(last_timestamp, pd.Timestamp) and (today - last_timestamp).days < 1:
                        logger.info(f"Skipping history fetch for {symbol}: Recent data exists")
                        return True
            except Exception as e:
                logger.warning(f"Error checking timestamp for {symbol}: {e}")
    ...
    try:
        ...
        # Ensure history_df has the correct timestamp format
        if 'timestamp' in history_df.columns:
            history_df['timestamp'] = pd.to_datetime(history_df['timestamp'])
        
        # Merge with existing price history if available
        if symbol in stocks_data:
            with price_history_lock:
                current_history = stocks_data[symbol]["price_history"]
                
                # Only keep real-time data from today
                if not current_history.empty:
                    try:
                        today = pd.Timestamp.now().floor('D')
                        # Convert timestamp column to datetime if needed
                        if 'timestamp' in current_history.columns:
                            if current_history['timestamp'].dtype == 'object':
                                current_history['timestamp'] = pd.to_datetime(current_history['timestamp'])
                        
                        today_data = current_history[current_history['timestamp'] >= today]
                        
                        # Combine historical data with today's data
                        if not today_data.empty:
                            combined_history = pd.concat([today_data, history_df], ignore_index=True)
                            combined_history = combined_history.drop_duplicates(subset=['timestamp'])
                            # Sort in chronological order (ascending) for proper analysis
                            combined_history = combined_history.sort_values('timestamp', ascending=True)
                            
                            # Update the price history
                            stocks_data[symbol]["price_history"] = combined_history.tail(MAX_PRICE_HISTORY_POINTS)
                        else:
                            stocks_data[symbol]["price_history"] = history_df.sort_values('timestamp', ascending=True)
                    except Exception as e:
                        logger.error(f"Error merging history for {symbol}: {e}")
                        stocks_data[symbol]["price_history"] = history_df.sort_values('timestamp', ascending=True)
                else:
                    stocks_data[symbol]["price_history"] = history_df.sort_values('timestamp', ascending=True)
                
                # Set flag to indicate data was loaded from Yahoo
                ...
                logger.info(f"Successfully loaded historical data for {symbol}: {len(history_df)} records")
                
                # Recalculate support/resistance with better historical data
                calculate_support_resistance(symbol)
                
                # Predict strategy with new historical data
                predict_strategy_for_stock(symbol)
...
def calculate_support_resistance(symbol):
    """
    Calculate support and resistance levels using historical data with enhanced algorithm
    to better identify key price levels
    """
    if symbol not in stocks_data:
        return
    ...
    if len(df) < 50:
        logger.warning(f"Not enough historical data for {symbol} to calculate reliable S/R levels")
        return
    
    try:
        # ...existing code for calculating support and resistance levels...
        
        # Update in the stock_info
        stock_info["support_levels"] = [round(level, 2) for level in support_levels]
        stock_info["resistance_levels"] = [round(level, 2) for level in resistance_levels]
        
        # Update the UI data store for the dashboard
        ui_data_store['stocks'][symbol] = ui_data_store.get('stocks', {}).get(symbol, {})
        ui_data_store['stocks'][symbol].update({
            'support_levels': stock_info["support_levels"],
            'resistance_levels': stock_info["resistance_levels"]
        })
        
        logger.info(f"Updated S/R levels for {symbol}: S={support_levels}, R={resistance_levels}")
    ...
    try:
        # Get all price data and ensure it's a Series
        ...
        if len(prices) < 50:
            logger.warning(f"Not enough valid price points for {symbol}")
            return
        
        # Make sure prices is sorted chronologically
        if 'timestamp' in df.columns:
            prices = df.sort_values('timestamp')['price'].dropna()
        
        # Use fractals to identify swing highs and lows
        swing_highs = []
        swing_lows = []
        
        # Find swing points (local maxima and minima)
        # More sophisticated than simple peaks - using a window approach
        window_size = min(21, len(prices) // 5)  # Adaptive window size
        if window_size < 2:
            window_size = 2
        ...
        for i in range(window_size, len(prices) - window_size):
            ...
            # Compare with points on either side within the window
            ...
            # If it's a swing point, add to the appropriate list
            if is_high:
        ...
        # Process swing highs to find resistance levels above current price
        resistance_levels = [level for level in swing_highs if level > current_price]
        
        # Process swing lows to find support levels below current price
        support_levels = [level for level in swing_lows if level < current_price]
        
        # If we don't have enough levels, use price clusters as additional levels
        if len(resistance_levels) < 2 or len(support_levels) < 2:
            # Round prices to identify price clusters
            round_factor = 0.5 if current_price < 100 else (5 if current_price < 1000 else 50)
            rounded_prices = (prices / round_factor).round() * round_factor
            value_counts = rounded_prices.value_counts()
            
            # Find price levels with significant activity (high frequency)
            price_clusters = value_counts[value_counts > len(prices) * 0.03].index.tolist()
            
            # Add these as potential S/R levels
            for level in price_clusters:
        ...
        # Sort and filter levels to get the closest ones
        support_levels = sorted(support_levels, reverse=True)  # Higher supports first
        resistance_levels = sorted(resistance_levels)  # Lower resistances first
        
        # Keep only the 3 closest levels in each direction
...
def fetch_history_from_yahoo(symbol, period="3mo"):
    ...
    Args:
        symbol (str): Stock symbol to fetch history for
        period (str): Period to fetch (e.g., "3mo" for 3 months)
    ...
    try:
        ...
        if symbol.upper() in ["NIFTY", "BANKNIFTY", "FINNIFTY"]:
            # For indices, we need special mappings
