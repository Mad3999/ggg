...
# Update the constant to ensure 30-second news checks
NEWS_CHECK_INTERVAL = 30  # Check for news every 30 seconds

def fetch_news_from_sources():
    """
    Fetch financial news from various free sources with proper timestamps
    ...
    try:
        ...
        if 'feedparser' in sys.modules:
            ...
            try:
                ...
                if yahoo_feed.entries:
                    for entry in yahoo_feed.entries[:15]:
                    ...
                    sources_fetched.append("Yahoo Finance")
                    logger.info(f"Successfully fetched {len(yahoo_feed.entries)} news items from Yahoo Finance")
                else:
                    logger.warning("Yahoo Finance feed returned no entries")
            except Exception as e:
                logger.warning(f"Error fetching Yahoo Finance news: {e}")
            
            # Add Moneycontrol news (India specific) - with multiple URL options
            mc_urls = [
            ...
            for mc_url in mc_urls:
                try:
                    ...
                    if mc_feed.entries:
                        for entry in mc_feed.entries[:15]:
                            # Try to parse the published date from the feed
                            try:
                                if hasattr(entry, 'published_parsed') and entry.published_parsed:
                                    item_time = datetime(*entry.published_parsed[:6])
                        ...
                        sources_fetched.append("Moneycontrol")
                        logger.info(f"Successfully fetched {len(mc_feed.entries)} news items from Moneycontrol ({mc_url})")
                        break  # Exit the loop if successful
                    else:
                        logger.warning(f"Moneycontrol feed returned no entries for URL: {mc_url}")
                except Exception as e:
                    logger.warning(f"Error fetching Moneycontrol news from {mc_url}: {e}")
            
            # Add Economic Times news (India specific) - with multiple URL options
            et_urls = [
            ...
            for et_url in et_urls:
                try:
                    et_feed = feedparser.parse(et_url)
                    if et_feed.entries:
                        for entry in et_feed.entries[:15]:
                            # Try to parse the published date from the feed
                            try:
                                if hasattr(entry, 'published_parsed') and entry.published_parsed:
                                    item_time = datetime(*entry.published_parsed[:6])
                        ...
                        sources_fetched.append("Economic Times")
                        logger.info(f"Successfully fetched {len(et_feed.entries)} news items from Economic Times ({et_url})")
                        break  # Exit the loop if successful
                    else:
                        logger.warning(f"Economic Times feed returned no entries for URL: {et_url}")
                except Exception as e:
                    logger.warning(f"Error fetching Economic Times news from {et_url}: {e}")
            
            # Add Business Standard as an additional source
            try:
                bs_feed = feedparser.parse('https://www.business-standard.com/rss/markets-106.rss')
                if bs_feed.entries:
                    for entry in bs_feed.entries[:15]:
                        # Try to parse the published date from the feed
                        try:
                            if hasattr(entry, 'published_parsed') and entry.published_parsed:
                                item_time = datetime(*entry.published_parsed[:6])
                    ...
                    sources_fetched.append("Business Standard")
                    logger.info(f"Successfully fetched {len(bs_feed.entries)} news items from Business Standard")
                else:
                    logger.warning("Business Standard feed returned no entries")
            except Exception as e:
                logger.warning(f"Error fetching Business Standard news: {e}")
            
            # Log summary of sources fetched
            if sources_fetched:
                logger.info(f"Successfully fetched latest news from sources: {', '.join(sources_fetched)}")
            else:
        ...
        logger.info(f"Fetched {len(news_items)} latest news items total")
        return news_items
    except Exception as e:
...
def update_news_data():
    ...
    current_time = datetime.now()
    # Ensure we check every 30 seconds
...
def diagnose_news_sources():
    """Test connection to news sources and report results"""
    ...
    for name, url in news_urls.items():
        try:
            # Check if URL is accessible with a timeout of 5 seconds
            response = urllib.request.urlopen(url, timeout=5)
            if response.getcode() == 200:
                content = response.read(1000)  # Read first 1000 bytes to check format
                results[name] = {
                    "status": "Success", 
                ...
                results[name] = {"status": "Failed", "code": response.getcode()}
        except Exception as e:
...
def find_option_token_in_json(symbol, strike, option_type, expiry_date=None):
    ...
    return None
# Add this global declaration at the top of the script, near other global variable declarations
tokens_and_symbols_df = None

def fetch_options_from_csv(stock_name, target_strike=None, csv_path=r"C:\Users\madhu\Pictures\ubuntu\stocks_and_options.csv"):
    """
    Fetch options for a specific stock with the closest expiry and near a given strike price.
    
    Args:
        stock_name (str): Name of the stock (e.g., NIFTY, BANKNIFTY, RELIANCE)
    ...
    Returns:
        dict: Dictionary with CE and PE options data
    """
    global tokens_and_symbols_df, stocks_data
    
    logger.info(f"Fetching options for {stock_name} near strike {target_strike}")
    ...
    try:
        ...
        if target_strike is None and stock_name in stocks_data:
            target_strike = stocks_data[stock_name].get("ltp")
            if not target_strike:
                logger.warning(f"No current price available for {stock_name}")
                # Use a default value or return
                return {"CE": None, "PE": None}
        elif target_strike is None:
            logger.warning(f"No strike price provided and {stock_name} not found in stocks_data")
            return {"CE": None, "PE": None}
        
        # Filter options for the specified stock
        stock_options = tokens_and_symbols_df[
            (tokens_and_symbols_df['exch_seg'] == 'NFO') & 
            (tokens_and_symbols_df['name'] == stock_name)
        ]
        
        if stock_options.empty:
            logger.warning(f"No options found for {stock_name}")
            return {"CE": None, "PE": None}
        ...
        if 'expiry' in stock_options.columns:
            ...
            if not sorted_expirations:
                logger.warning(f"No expiry dates found for {stock_name}")
                return {"CE": None, "PE": None}
            ...
            return {"CE": ce_result, "PE": pe_result}
        else:
        ...
        return {"CE": None, "PE": None}
def update_options_from_csv(symbol, target_strike=None):
    """
    Update options data for a specific stock using CSV data.
    ...
    # Fetch options from CSV
    options = fetch_options_from_csv(symbol, target_strike)
    ...
    if broker_connected:
        if result["CE"]:
            fetch_option_data(result["CE"])
        if result["PE"]:
            fetch_option_data(result["PE"])
...
def find_and_add_atm_options(symbol, current_price):
    """
    Find and add ATM options for a stock using CSV data.
...
def update_option_selection(force_update=False):
    ...
    with enhanced expiry and strike selection
    """
...
def _should_update_options(symbol, current_price):
    """
    Determine if options should be updated based on multiple criteria
    ...
    Returns:
        bool: Whether options should be updated
    """
    ...
    if ce_distance > strike_interval * 2 or pe_distance > strike_interval * 2:
        logger.info(f"Updating options for {symbol}: Current strikes too far from ATM")
        return True
    
    # Check if options are using fallback data
    if (ce_details.get("using_fallback", False) or 
        pe_details.get("using_fallback", False)):
        logger.info(f"Updating options for {symbol}: Current options using fallback data")
        return True
...
def update_market_sentiment():
    ...
    for symbol, stock_info in stocks_data.items():
        # Skip indices from sentiment calculation
        if symbol in ["overall"]:
            continue
        ...
        try:
            ...
            # 1. RSI component - improved weighting
            if rsi > 70:
            ...
            # 2. MACD component - improved with trend detection
            if histogram > 0:
                # Positive histogram
            ...
            try:
                ...
                logger.warning(f"Error calculating price action: {e}")
            ...
            logger.debug(f"Sentiment for {symbol}: {market_sentiment[symbol]} (Score: {sentiment_score:.2f})")
...
def enter_trade(option_key, strategy_type, trade_source="TECHNICAL"):
    ...
    if strategy_type == "SCALP":
        stop_loss_pct = 0.02   # 2.0% for scalping (increased from 1.5%)
    ...
    trading_state.last_sl_adjustment_price[option_key] = current_price  # Initialize tracking price
...
def should_exit_trade(option_key):
    ...
    if time_since_entry > 180 and pnl_pct > 0 and trading_state.trailing_sl_activated.get(option_key, False):
...
def update_dynamic_stop_loss(option_key):
    ...
    that adapts to changing market conditions and triggers after 10 points movement
    """
    ...
    # Initialize the tracking of last price where SL was adjusted
    if not hasattr(trading_state, 'last_sl_adjustment_price'):
        trading_state.last_sl_adjustment_price = {}
    
    # Initialize for this option if not already set
    if option_key not in trading_state.last_sl_adjustment_price:
        trading_state.last_sl_adjustment_price[option_key] = entry_price
    
    # Calculate price movement since last adjustment
    last_adjustment_price = trading_state.last_sl_adjustment_price[option_key]
...
def exit_trade(option_key, reason="Manual"):
    ...
    return True
def initialize_symbol_tracking():
    """
    Initialize and cache symbol tracking data from CSV
    Loads symbols only once and provides efficient lookup methods
    """
    global tokens_and_symbols_df, stocks_symbol_cache, options_symbol_cache
    ...
    try:
        ...
        # Create symbol caches
        stocks_symbol_cache = {}
        options_symbol_cache = {}
        
        # Populate stock symbol cache (NSE stocks)
        stocks_data = tokens_and_symbols_df[
            (tokens_and_symbols_df['exch_seg'] == 'NSE') & 
            (tokens_and_symbols_df['strike'].isna())
        ]
        
        for _, row in stocks_data.iterrows():
            stocks_symbol_cache[row['symbol']] = {
                'token': row['token'],
                'name': row['name'],
                'exchange': row['exch_seg']
            }
        
        # Populate options symbol cache (NFO options)
        options_data = tokens_and_symbols_df[
            (tokens_and_symbols_df['exch_seg'] == 'NFO') & 
            (tokens_and_symbols_df['strike'].notna())
        ]
        
        for _, row in options_data.iterrows():
            options_symbol_cache[row['symbol']] = {
                'token': row['token'],
                'name': row['name'],
                'strike': row['strike'],
                'expiry': row['expiry'],
        ...
        logger.info(f"Initialized symbol caches: {len(stocks_symbol_cache)} stocks, {len(options_symbol_cache)} options")
...
def get_stock_symbol_details(symbol):
    """
    Retrieve stock symbol details from cache
    
    Args:
        symbol (str): Stock symbol
    
    Returns:
        dict: Symbol details or None
    """
    return stocks_symbol_cache.get(symbol.upper())

def get_option_symbol_details(symbol):
    """
    Retrieve option symbol details from cache
    ...
    """
    return options_symbol_cache.get(symbol.upper())

def remove_stock_symbol(symbol):
    """
    Remove a stock symbol from the cache
    
    Args:
        symbol (str): Stock symbol to remove
    """
    symbol = symbol.upper()
    if symbol in stocks_symbol_cache:
        del stocks_symbol_cache[symbol]
        logger.info(f"Removed stock symbol {symbol} from cache")

def remove_option_symbol(symbol):
    """
    Remove an option symbol from the cache
    
    Args:
        symbol (str): Option symbol to remove
    """
    symbol = symbol.upper()
    if symbol in options_symbol_cache:
        del options_symbol_cache[symbol]
        logger.info(f"Removed option symbol {symbol} from cache")

def fetch_bulk_data_with_cached_symbols(symbols):
    """
    Fetch bulk data using cached symbol information
    
    Args:
        symbols (list): List of symbols to fetch
    
    Returns:
        dict: Fetched data with cached symbol details
    """
    results = {
        "stocks": {},
        "options": {}
    ...
    for symbol in symbols:
        symbol_details = get_stock_symbol_details(symbol)
        if symbol_details:
            # Fetch stock data using cached token
            stock_data = fetch_stock_data_by_token(
                symbol, 
                symbol_details['token'], 
                symbol_details['exchange']
            )
            if stock_data:
        ...
        if symbol in stocks_data:
            # Find and fetch associated options
            ce_symbol = f"{symbol}CE"
            pe_symbol = f"{symbol}PE"
            
            ce_details = get_option_symbol_details(ce_symbol)
            pe_details = get_option_symbol_details(pe_symbol)
            
            if ce_details:
                ce_data = fetch_option_data_by_token(ce_symbol, ce_details['token'])
                if ce_data:
                    results["options"][ce_symbol] = ce_data
            
            if pe_details:
                pe_data = fetch_option_data_by_token(pe_symbol, pe_details['token'])
                if pe_data:
                    results["options"][pe_symbol] = pe_data
    
    return results


# Initialize symbol tracking when the module is first imported
initialize_symbol_tracking()
...
def update_ui_data_store(n_intervals):
    ...
    # Create a deep copy of current UI data to properly compare changes
    current_ui_data = {}
    ...
    # Process stock data with advanced change detection
    for symbol in stocks_data:
        stock_info = stocks_data[symbol]
        ...
        # Compare LTP for change
        prev_price = current_stock_data.get('price')
        if prev_price != ltp:
            new_stock_data['price'] = ltp
            needs_update = True
            new_stock_data['has_changed'] = True
            new_stock_data['change_direction'] = 'up' if prev_price is not None and ltp > prev_price else 'down'
        else:
            new_stock_data['price'] = prev_price
            new_stock_data['has_changed'] = False
            new_stock_data['change_direction'] = current_stock_data.get('change_direction', 'none')
            
        # Compare other fields for change
        if current_stock_data.get('change') != stock_info.get("change_percent", 0):
            new_stock_data['change'] = stock_info.get("change_percent", 0)
            needs_update = True
        else:
            new_stock_data['change'] = current_stock_data.get('change', 0)
            
        # Check OHLC data
        current_ohlc = current_stock_data.get('ohlc', {})
        new_ohlc = {
            'open': stock_info.get("open"),
            'high': stock_info.get("high"),
            'low': stock_info.get("low"),
        ...
        if current_ohlc != new_ohlc:
            new_stock_data['ohlc'] = new_ohlc
            needs_update = True
        else:
            new_stock_data['ohlc'] = current_ohlc
            
        # Check update time
        if current_stock_data.get('last_updated') != last_updated_str:
            new_stock_data['last_updated'] = last_updated_str
            needs_update = True
        else:
            new_stock_data['last_updated'] = current_stock_data.get('last_updated')
        
        # Only add to result if something changed or it's a new stock
        if needs_update or symbol not in current_ui_data.get('stocks', {}):
            result['stocks'][symbol] = new_stock_data
        else:
            result['stocks'][symbol] = current_stock_data
    
    # Process options data with similar change detection
    for symbol in stocks_data:
        ...
        if ce_key and ce_key in options_data:
            ...
            if prev_ce_price != ce_price:
                new_ce_data['price'] = ce_price
                new_ce_data['has_changed'] = True
                new_ce_data['change_direction'] = 'up' if prev_ce_price is not None and ce_price > prev_ce_price else 'down'
                needs_update = True
            else:
                new_ce_data['price'] = prev_ce_price
                new_ce_data['has_changed'] = False
                new_ce_data['change_direction'] = current_ce.get('change_direction', 'none')
            
            # Check other fields
            if current_ce.get('signal') != ce_option.get('signal', 0):
                new_ce_data['signal'] = ce_option.get('signal', 0)
                needs_update = True
            else:
                new_ce_data['signal'] = current_ce.get('signal', 0)
                
            if current_ce.get('strength') != ce_option.get('strength', 0):
                new_ce_data['strength'] = ce_option.get('strength', 0)
                needs_update = True
            else:
                new_ce_data['strength'] = current_ce.get('strength', 0)
                
            if current_ce.get('trend') != ce_option.get('trend', 'NEUTRAL'):
                new_ce_data['trend'] = ce_option.get('trend', 'NEUTRAL')
                needs_update = True
            else:
                new_ce_data['trend'] = current_ce.get('trend', 'NEUTRAL')
                
            if current_ce.get('using_fallback') != ce_option.get('using_fallback', False):
                new_ce_data['using_fallback'] = ce_option.get('using_fallback', False)
                needs_update = True
            else:
                new_ce_data['using_fallback'] = current_ce.get('using_fallback', False)
                
            if current_ce.get('strike') != ce_option.get('strike', 'N/A'):
                new_ce_data['strike'] = ce_option.get('strike', 'N/A')
                needs_update = True
            else:
                new_ce_data['strike'] = current_ce.get('strike', 'N/A')
            
            # Only update if needed
            if needs_update or 'ce' not in current_option_data:
                result['options'][symbol]['ce'] = new_ce_data
            else:
                result['options'][symbol]['ce'] = current_ce
        
        # Process PE options with the same approach
        pe_key = stocks_data[symbol].get("primary_pe")
        if pe_key and pe_key in options_data:
            pe_option = options_data[pe_key]
            ...
            if prev_pe_price != pe_price:
                new_pe_data['price'] = pe_price
                new_pe_data['has_changed'] = True
                new_pe_data['change_direction'] = 'up' if prev_pe_price is not None and pe_price > prev_pe_price else 'down'
                needs_update = True
            else:
                new_pe_data['price'] = prev_pe_price
                new_pe_data['has_changed'] = False
                new_pe_data['change_direction'] = current_pe.get('change_direction', 'none')
            
            # Check other fields
            if current_pe.get('signal') != pe_option.get('signal', 0):
                new_pe_data['signal'] = pe_option.get('signal', 0)
                needs_update = True
            else:
                new_pe_data['signal'] = current_pe.get('signal', 0)
                
            if current_pe.get('strength') != pe_option.get('strength', 0):
                new_pe_data['strength'] = pe_option.get('strength', 0)
                needs_update = True
            else:
                new_pe_data['strength'] = current_pe.get('strength', 0)
                
            if current_pe.get('trend') != pe_option.get('trend', 'NEUTRAL'):
                new_pe_data['trend'] = pe_option.get('trend', 'NEUTRAL')
                needs_update = True
            else:
                new_pe_data['trend'] = current_pe.get('trend', 'NEUTRAL')
                
            if current_pe.get('using_fallback') != pe_option.get('using_fallback', False):
                new_pe_data['using_fallback'] = pe_option.get('using_fallback', False)
                needs_update = True
            else:
                new_pe_data['using_fallback'] = current_pe.get('using_fallback', False)
                
            if current_pe.get('strike') != pe_option.get('strike', 'N/A'):
                new_pe_data['strike'] = pe_option.get('strike', 'N/A')
                needs_update = True
            else:
                new_pe_data['strike'] = current_pe.get('strike', 'N/A')
            
            # Only update if needed
            if needs_update or 'pe' not in current_option_data:
                result['options'][symbol]['pe'] = new_pe_data
            else:
                result['options'][symbol]['pe'] = current_pe
    
    # Process PCR data with similar approach
    for symbol, data in pcr_data.items():
        if symbol in stocks_data:
            current_pcr = current_ui_data.get('pcr', {}).get(symbol, {})
            
            needs_update = False
            new_pcr_data = {}
            
            if current_pcr.get('current') != data['current']:
                new_pcr_data['current'] = data['current']
                needs_update = True
            else:
                new_pcr_data['current'] = current_pcr.get('current', 1.0)
                
            if current_pcr.get('trend') != data['trend']:
                new_pcr_data['trend'] = data['trend']
                needs_update = True
            else:
                new_pcr_data['trend'] = current_pcr.get('trend', 'NEUTRAL')
                
            if current_pcr.get('strength') != data['strength']:
                new_pcr_data['strength'] = data['strength']
                needs_update = True
            else:
                new_pcr_data['strength'] = current_pcr.get('strength', 0)
            
            if needs_update or symbol not in current_ui_data.get('pcr', {}):
                result['pcr'][symbol] = new_pcr_data
            else:
...
@app.callback(
    ...
    [Input('slow-interval', 'n_intervals'),  # Changed to slow interval - updates less frequently
...
def update_stock_cards(n_intervals, add_clicks, remove_clicks):
    """Create stock cards with better performance by reducing recreations"""
...
@app.callback(
    ...
    [
        ...
        State({"type": "stock-price", "index": ALL}, "children"),
        State({"type": "stock-price", "index": ALL}, "className"),
        State({"type": "stock-change", "index": ALL}, "children"),
        State({"type": "stock-change", "index": ALL}, "className"),
        State({"type": "stock-ohlc", "index": ALL}, "children"),
        State({"type": "stock-pcr", "index": ALL}, "children"),
        State({"type": "pcr-strength", "index": ALL}, "children"),
        State({"type": "stock-sentiment", "index": ALL}, "children"),
        State({"type": "stock-sentiment", "index": ALL}, "className"),
        State({"type": "stock-sr-levels", "index": ALL}, "children"),
        State({"type": "stock-last-update", "index": ALL}, "children"),
        State({"type": "strategy-prediction", "index": ALL}, "children")
    ]
)
def update_stocks_display(data, 
                         badge_ids, 
                         current_prices, current_price_classes,
                         current_changes, current_change_classes,
                         current_ohlc, current_pcr_values,
                         current_pcr_strength, current_sentiment_texts,
                         current_sentiment_classes, current_sr_levels,
                         current_last_updates, current_strategy_predictions):
    """Update stock display values with intelligent state management to prevent blinking"""
    ...
    if not data:
        ...
        empty_result = [
            ...
            ["fs-4 text-light smooth-transition price-value"] * len(badge_ids),  # price class outputs
    ...
    # Get symbols from the badge IDs
    symbols = [id_dict["index"] for id_dict in badge_ids]
    
    # Process each symbol
    for i, symbol in enumerate(symbols):
        # Get current values from state (to preserve if unchanged)
        current_price = current_prices[i] if i < len(current_prices) else "Loading..."
        current_price_class = current_price_classes[i] if i < len(current_price_classes) else "fs-4 text-light price-value"
        current_change = current_changes[i] if i < len(current_changes) else "0.00%"
        current_change_class = current_change_classes[i] if i < len(current_change_classes) else "text-secondary price-change"
        current_ohlc_value = current_ohlc[i] if i < len(current_ohlc) else "OHLC data not available"
        current_pcr_value = current_pcr_values[i] if i < len(current_pcr_values) else "N/A"
        current_pcr_strength_value = current_pcr_strength[i] if i < len(current_pcr_strength) else ""
        current_sentiment_text = current_sentiment_texts[i] if i < len(current_sentiment_texts) else "NEUTRAL"
        current_sentiment_class = current_sentiment_classes[i] if i < len(current_sentiment_classes) else "badge bg-secondary"
        current_sr_level = current_sr_levels[i] if i < len(current_sr_levels) else "S/R not available"
        current_last_update = current_last_updates[i] if i < len(current_last_updates) else "Not yet updated"
        current_strategy_prediction = current_strategy_predictions[i] if i < len(current_strategy_predictions) else ""
        
        if symbol in stocks_data:
            stock_info = stocks_data[symbol]
            stock_data = data.get('stocks', {}).get(symbol, {})
            
            # 1. Source Badge - only changes with broker connection
            badge_text = "Live" if broker_connected else "Offline"
            badge_class = "badge bg-success ms-2 small smooth-transition" if broker_connected else "badge bg-warning text-dark ms-2 small smooth-transition"
            source_badges.append(badge_text)
            source_classes.append(badge_class)
            
            # 2. Price with intelligent highlight management
            ltp = stock_data.get('price')
            if ltp is not None:
                price_text = f"₹{ltp:.2f}"
                
                # Only update if changed
                if price_text != current_price:
                    price_outputs.append(price_text)
                    
                    # Apply highlight only for actual changes
                    base_class = "fs-4 text-light price-value smooth-transition"
                    if stock_data.get('has_changed', False):
                        if stock_data.get('change_direction') == 'up':
                            price_classes.append(f"{base_class} highlight-positive")
                        elif stock_data.get('change_direction') == 'down':
                            price_classes.append(f"{base_class} highlight-negative")
                        else:
                            price_classes.append(base_class)
                    else:
                        price_classes.append(base_class)
                else:
                    # Keep current price
                    price_outputs.append(current_price)
                    
                    # Handle highlight classes - remove after animation completes
                    if 'highlight-positive' in current_price_class or 'highlight-negative' in current_price_class:
                        # Strip out the highlight classes to prevent accumulation
                        base_classes = []
                        for cls in current_price_class.split():
                            if 'highlight' not in cls:
                                base_classes.append(cls)
                        price_classes.append(' '.join(base_classes))
                    else:
                        price_classes.append(current_price_class)
            else:
                price_outputs.append("Waiting...")
                price_classes.append("fs-4 text-light price-value smooth-transition")
            
            # 3. Change Percentage with appropriate color
            change_pct = stock_data.get('change', 0)
            if change_pct > 0:
                change_text = f"+{change_pct:.2f}%"
                change_class = "text-success price-change smooth-transition"
            elif change_pct < 0:
                change_text = f"{change_pct:.2f}%"
                change_class = "text-danger price-change smooth-transition"
            else:
                change_text = "0.00%"
                change_class = "text-secondary price-change smooth-transition"
            
            # Only update if value changed
            if change_text != current_change:
                change_text_outputs.append(change_text)
                change_class_outputs.append(change_class)
            else:
                change_text_outputs.append(current_change)
                change_class_outputs.append(current_change_class)
            ...
            if all(x is not None for x in [
                ...
                # Only update if changed
                if ohlc_text != current_ohlc_value:
                    ohlc_outputs.append(ohlc_text)
                else:
                    ohlc_outputs.append(current_ohlc_value)
            else:
                ohlc_outputs.append("OHLC data not available")
            
            # 5. PCR Value
            pcr_value = data.get('pcr', {}).get(symbol, {}).get('current', 1.0)
            pcr_text = f"{pcr_value:.2f}"
            
            # Only update if changed
            if pcr_text != current_pcr_value:
                pcr_outputs.append(pcr_text)
            else:
                pcr_outputs.append(current_pcr_value)
            
            # 6. PCR Strength Indicator
            pcr_strength = data.get('pcr', {}).get(symbol, {}).get('strength', 0)
            if pcr_strength > 0.5:
            ...
            # 7. Sentiment with appropriate badge
            sentiment = data.get('sentiment', {}).get(symbol, "NEUTRAL")
            
            # Only update if changed
            if sentiment != current_sentiment_text:
                ...
                if "BULLISH" in sentiment:
                    if "STRONGLY" in sentiment:
                        sentiment_class = f"{base_sentiment_class} bg-success"
                    else:
                        sentiment_class = f"{base_sentiment_class} bg-success bg-opacity-75"
                elif "BEARISH" in sentiment:
                    if "STRONGLY" in sentiment:
                        sentiment_class = f"{base_sentiment_class} bg-danger"
                    else:
                        sentiment_class = f"{base_sentiment_class} bg-danger bg-opacity-75"
                else:
                    sentiment_class = f"{base_sentiment_class} bg-secondary"
                
                sentiment_class_outputs.append(sentiment_class)
            else:
                sentiment_text_outputs.append(current_sentiment_text)
                sentiment_class_outputs.append(current_sentiment_class)
            ...
            if support_levels and resistance_levels:
                ...
                # Only update if changed
                if sr_text != current_sr_level:
                    sr_levels_outputs.append(sr_text)
                else:
                    sr_levels_outputs.append(current_sr_level)
            else:
                sr_levels_outputs.append("S/R not available")
            
            # 9. Last Update Time
            update_text = f"Updated: {stock_data.get('last_updated', 'N/A')}"
            
            # Only update if changed
            if update_text != current_last_update:
                last_update_outputs.append(update_text)
            else:
                last_update_outputs.append(current_last_update)
            
            # 10. Strategy Prediction
            predicted_strategy = stock_info.get("predicted_strategy")
            strategy_confidence = stock_info.get("strategy_confidence", 0)
            
            # Formulate prediction text
...
def update_options_display(data, ce_strike_ids):
    ...
    for symbol in symbols:
        if symbol in stocks_data:
            ...
            if ce_data:
                # Get the primary CE option key for this stock
                ce_option_key = stock_info.get("primary_ce")
                
                # Get the full option symbol if available in options_data
                option_symbol = "N/A"
                if ce_option_key and ce_option_key in options_data:
                    option_info = options_data[ce_option_key]
                    # Try to get the full symbol from options_data
                    option_symbol = option_info.get("symbol", "N/A")
                    
                    # If the symbol exists but looks like a broker-specific ID, create a readable format
                    if option_symbol == "N/A" or len(option_symbol) < 5:
                        # Build a formatted symbol from components
                        parent = option_info.get("parent_symbol", symbol)
                        strike = option_info.get("strike", "N/A")
                        expiry = option_info.get("expiry", "")
                        
                        # Format expiry to standard format if available
                        if expiry:
                            # Try to parse various expiry formats
                            try:
                                if "-" in expiry:  # Format like "27-MAR-25"
                                    parts = expiry.split("-")
                                    if len(parts) == 3:
                                        expiry = f"{parts[0]}{parts[1]}{parts[2][-2:]}"  # Convert to 27MAR25
                                # More expiry parsing could be added here
                            except:
                                # Keep original if parsing fails
                                pass
                                
                        # Format strike as integer if it's a whole number
                        try:
                        ...
                        # Create readable option symbol
                        option_symbol = f"{parent}{expiry}{strike}CE"
                
                # Use the full option symbol for display
                ce_strike_outputs.append(option_symbol)
            ...
            if pe_data:
                # Get the primary PE option key for this stock
                pe_option_key = stock_info.get("primary_pe")
                
                # Get the full option symbol if available in options_data
                option_symbol = "N/A"
                if pe_option_key and pe_option_key in options_data:
                    option_info = options_data[pe_option_key]
                    # Try to get the full symbol from options_data
                    option_symbol = option_info.get("symbol", "N/A")
                    
                    # If the symbol doesn't exist or is a broker-specific ID, create a readable format
                    if option_symbol == "N/A" or len(option_symbol) < 5:
                        # Build a formatted symbol from components
                        parent = option_info.get("parent_symbol", symbol)
                        strike = option_info.get("strike", "N/A")
                        ...
                        # Create readable option symbol
                        option_symbol = f"{parent}{expiry}{strike}PE"
                
                # Use the full option symbol for display
                pe_strike_outputs.append(option_symbol)
